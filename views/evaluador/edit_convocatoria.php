<?php
// Asume que $convocatoria, $areas y $_SESSION están definidos en tu controlador
?>
<!DOCTYPE html>
<html lang="es">
<head>
    ...
    <title>Editar Convocatoria - PostulaPe</title>
    ...
    <style>
        ...
        @media (max-width: 768px) {
            ...
            .card { padding: 1rem; }
            h3 { font-size: 1.5rem; }
            .form-control, .form-select, .date-input { padding: 0.5rem; font-size: 0.9rem; }
...
<body>
    <div class="container">
        <div class="card">
            ...
            <form action="index.php?controller=evaluador&action=editConvocatoria&id=<?= $convocatoria['IdConvocatoria']; ?>" method="POST">
                <div class="mb-3">
                    <label for="titulo" class="form-label">Nombre Convocatoria</label>
                    <input type="text" class="form-control" id="titulo" name="titulo" 
                           value="<?= htmlspecialchars($convocatoria['Titulo']); ?>" required>
                </div>
                ...
                <div class="mb-3">
                    ...
                    <select name="idArea" class="form-select" required>
                        ...
                        <?php foreach ($areas as $area): ?>
                            <option value="<?= $area['IdArea']; ?>" <?= ($convocatoria['IdArea'] == $area['IdArea']) ? 'selected' : '' ?>>
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control" id="descripcion" name="descripcion"><?= htmlspecialchars($convocatoria['Descripcion']); ?></textarea>
                </div>

                <div class="mb-3 row justify-content-center g-1">
                    <div class="col-auto text-center">
                        <label for="fechaInicio" class="form-label">Fecha Inicio</label>
                        <input type="date" class="form-control date-input" id="fechaInicio" name="fechaInicio" 
                               value="<?= $convocatoria['FechaInicio']; ?>" required>
                    </div>
                    <div class="col-auto text-center">
                        <label for="fechaFin" class="form-label">Fecha Fin</label>
                        <input type="date" class="form-control date-input" id="fechaFin" name="fechaFin" 
                               value="<?= $convocatoria['FechaFin']; ?>" required>
                    </div>
                </div>
