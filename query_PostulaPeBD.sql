...
CREATE TABLE Usuario (
    ...
    ContrasenaHash varbinary(128) NOT NULL,  -- hash encriptado (ej. SHA2-512)
...
CREATE TABLE EstadoCivil (
    ...
    TipoEstadoCivil VARCHAR(20) NOT NULL
);
...
CREATE TABLE Postulante (
    ...
    Telefono VARCHAR(20),
    Celular VARCHAR(20),
    Foto LONGBLOB, -- foto en binario
    IdEliminado TINYINT DEFAULT 0,
    IdUsuarioCreacion INT,
    FechaCreacion DATETIME DEFAULT CURRENT_TIMESTAMP,
    IdUsuarioModificacion INT,
    FechaModificacion DATETIME,
    CONSTRAINT FK_Postulante_Usuario FOREIGN KEY (IdUsuario) REFERENCES Usuario(IdUsuario),
    CONSTRAINT FK_Postulante_TipoDocumento FOREIGN KEY (IdTipoDocumento) REFERENCES TipoDocumento(IdTipoDocumento),
    CONSTRAINT FK_Postulante_Genero FOREIGN KEY (IdGenero) REFERENCES Genero(IdGenero),
    CONSTRAINT FK_Postulante_EstadoCivil FOREIGN KEY (IdEstadoCivil) REFERENCES EstadoCivil(IdEstadoCivil),
    CONSTRAINT FK_Postulante_Departamento FOREIGN KEY (IdDepartamento) REFERENCES Departamento(IdDepartamento),
    CONSTRAINT FK_Postulante_Provincia FOREIGN KEY (IdProvincia) REFERENCES Provincia(IdProvincia),
    CONSTRAINT FK_Postulante_Distrito FOREIGN KEY (IdDistrito) REFERENCES Distrito(IdDistrito)
);
...
CREATE TABLE Convocatoria (
    ...
    IdUsuarioModificacion INT,
    FechaModificacion DATETIME,
    CONSTRAINT FK_Convocatoria_Area FOREIGN KEY (IdArea) REFERENCES Area(IdArea)
);
...
CREATE TABLE Postulacion (
    ...
    IdPostulante INT,
    IdConvocatoria INT,
    Estado ENUM('postulado','en_proceso','seleccionado','descartado') DEFAULT 'postulado',
    CVPath VARCHAR(255), -- Ruta del CV específico por postulación
    IdEliminado TINYINT DEFAULT 0,
    IdUsuarioCreacion INT,
    FechaCreacion DATETIME DEFAULT CURRENT_TIMESTAMP,
    IdUsuarioModificacion INT,
    FechaModificacion DATETIME,
    CONSTRAINT FK_Postulacion_Postulante FOREIGN KEY (IdPostulante) REFERENCES Postulante(IdPostulante),
    CONSTRAINT FK_Postulacion_Convocatoria FOREIGN KEY (IdConvocatoria) REFERENCES Convocatoria(IdConvocatoria)
);
...
CREATE TABLE Evaluacion (
    ...
    IdUsuarioModificacion INT,
    FechaModificacion DATETIME,
    CONSTRAINT FK_Evaluacion_Postulacion FOREIGN KEY (IdPostulacion) REFERENCES Postulacion(IdPostulacion),
    CONSTRAINT FK_Evaluacion_Etapa FOREIGN KEY (IdEtapa) REFERENCES Etapa(IdEtapa),
    CONSTRAINT FK_Evaluacion_Usuario FOREIGN KEY (IdEvaluador) REFERENCES Usuario(IdUsuario)
);

-- ===========================================
-- DATOS
-- ===========================================
INSERT INTO Perfil (Nombre, IdUsuarioCreacion, FechaCreacion) VALUES
    ('Administrador', 1, CURRENT_TIMESTAMP),
    ('Postulante', 1, CURRENT_TIMESTAMP),
    ('Evaluador', 1, CURRENT_TIMESTAMP);
    
    INSERT INTO Etapa (Nombre, IdUsuarioCreacion)
VALUES 
    ('entrevista', 1),
    ('pruebas técnicas', 1),
    ('psicológicas', 1);
    
INSERT INTO Area (Nombre, Descripcion, IdUsuarioCreacion, FechaCreacion)
VALUES
    ('Recursos Humanos', 'Área encargada de la gestión del personal y procesos de reclutamiento.', 1, CURRENT_TIMESTAMP()),
    ('Tecnología de la Información', 'Área responsable del soporte técnico y desarrollo de sistemas.', 1, CURRENT_TIMESTAMP()),
    ('Finanzas', 'Área que maneja la contabilidad, presupuestos y pagos de la empresa.', 1, CURRENT_TIMESTAMP()),
    ('Marketing', 'Área enfocada en la publicidad, promoción y posicionamiento de la empresa.', 1, CURRENT_TIMESTAMP()),
    ('Operaciones', 'Área que supervisa la producción y el cumplimiento de procesos operativos.', 1, CURRENT_TIMESTAMP());
    
INSERT INTO Area (Nombre, Descripcion, IdUsuarioCreacion, FechaCreacion)
VALUES
('Logística', 'Área encargada de la gestión de inventarios, transporte y distribución.', 1, CURRENT_TIMESTAMP()),
('Atención al Cliente', 'Área encargada de brindar soporte y resolver consultas de los clientes.', 1, CURRENT_TIMESTAMP()),
('Investigación y Desarrollo', 'Área enfocada en innovación y mejora de productos y servicios.', 1, CURRENT_TIMESTAMP()),
('Ventas', 'Área responsable de la comercialización de productos y servicios.', 1, CURRENT_TIMESTAMP()),
('Legal', 'Área encargada de asuntos jurídicos, contratos y cumplimiento normativo.', 1, CURRENT_TIMESTAMP());
    
INSERT INTO Convocatoria (Titulo, Descripcion, FechaInicio, FechaFin, IdArea, IdUsuarioCreacion)
VALUES
('Analista de Recursos Humanos',
 'Se busca un Analista de RRHH para gestionar procesos de selección, evaluación del personal y dar soporte en la implementación de políticas de gestión humana. El candidato participará en actividades de inducción, capacitación y acompañamiento en clima laboral.',
 '2025-09-01', '2025-09-30', 1, 1),

('Desarrollador Web Full Stack',
 'Convocatoria para desarrollador web con experiencia en PHP, JavaScript y MySQL. El profesional será responsable de crear, mantener y optimizar aplicaciones web, trabajar con frameworks modernos y colaborar estrechamente con el área de TI para garantizar escalabilidad y seguridad.',
 '2025-09-05', '2025-10-05', 2, 1),

('Contador Junior',
 'Se requiere contador junior para registro contable, conciliaciones bancarias y apoyo en la elaboración de estados financieros. Se valorará conocimientos en normativas tributarias locales, manejo de software contable y capacidad de análisis.',
 '2025-09-10', '2025-10-10', 3, 1),

('Especialista en Marketing Digital',
 'Búsqueda de especialista en marketing digital con experiencia en campañas de redes sociales, posicionamiento SEO y publicidad en línea. El puesto requiere desarrollar estrategias de comunicación digital, análisis de métricas y coordinación de campañas pagadas.',
 '2025-09-12', '2025-10-12', 4, 1),

('Supervisor de Operaciones',
 'Se necesita supervisor para coordinar la producción, supervisar equipos de trabajo y garantizar el cumplimiento de procesos operativos. Será responsable de proponer mejoras continuas y asegurar estándares de calidad en los entregables.',
 '2025-09-15', '2025-10-15', 5, 1),

('Coordinador de Logística',
 'Se busca un coordinador de logística responsable de supervisar el flujo de inventarios, coordinar la distribución de productos y garantizar la eficiencia en los procesos de transporte. El candidato deberá trabajar en conjunto con proveedores y áreas internas para asegurar el cumplimiento de tiempos y optimización de recursos.',
 '2025-09-18', '2025-10-18', 6, 1),

('Representante de Atención al Cliente',
 'Convocatoria dirigida a profesionales con habilidades de comunicación y orientación al cliente. El puesto implica atender consultas presenciales y virtuales, gestionar reclamos, brindar seguimiento a solicitudes y garantizar una experiencia de calidad en cada interacción. Se valorará experiencia previa en call centers o atención en puntos de servicio.',
 '2025-09-20', '2025-10-20', 7, 1),

('Investigador en Innovación',
 'Se requiere profesional para el área de Investigación y Desarrollo con experiencia en proyectos de innovación tecnológica. Entre sus funciones estará el análisis de nuevas tendencias de mercado, creación de prototipos, pruebas de concepto y mejora continua de los productos y servicios existentes. Se busca perfil creativo y con capacidad de trabajo en equipo multidisciplinario.',
 '2025-09-22', '2025-10-22', 8, 1),

('Ejecutivo de Ventas',
 'Buscamos un ejecutivo de ventas con orientación a resultados, capaz de identificar nuevas oportunidades comerciales, gestionar relaciones con clientes clave y cumplir objetivos de facturación. El rol incluye la elaboración de reportes de ventas, desarrollo de estrategias de negociación y participación activa en ferias y eventos comerciales.',
 '2025-09-25', '2025-10-25', 9, 1),

('Asesor Legal',
 'Convocatoria para profesional en derecho con experiencia en contratos, cumplimiento normativo y asesoría empresarial. El candidato deberá garantizar la legalidad de los procesos internos, representar a la organización en asuntos legales y elaborar informes jurídicos estratégicos. Se valorará experiencia previa en el sector corporativo o consultoría legal.',
 '2025-09-28', '2025-10-28', 10, 1);



-- Insertar Usuarios (contraseñas hasheadas con SHA2-512)
INSERT INTO Usuario (Usuario, ContrasenaHash, ApellidoPaterno, ApellidoMaterno, Nombre, IdPerfil, IdUsuarioCreacion, FechaCreacion) VALUES
    ('admin@system.com', SHA2('password123', 512), 'Gomez', 'Perez', 'Juan', 1, 1, CURRENT_TIMESTAMP),
    ('postulante1@system.com', SHA2('password123', 512), 'Lopez', 'Martinez', 'Maria', 2, 1, CURRENT_TIMESTAMP),
    ('evaluador1@system.com', SHA2('password123', 512), 'Rodriguez', 'Sanchez', 'Carlos', 3, 1, CURRENT_TIMESTAMP);


-- Insertar Tipos de Documento (para combo box)
INSERT INTO TipoDocumento (Abreviatura, NombreTipoDocumento) VALUES
    ('DNI', 'Documento Nacional de Identidad'),
    ('CE', 'Carné de Extranjería');

-- Insertar Géneros (para combo box)
INSERT INTO Genero (TipoGenero) VALUES
    ('Masculino'),
    ('Femenino');

-- Insertar Estados Civiles (para combo box)
INSERT INTO EstadoCivil (TipoEstadoCivil) VALUES
    ('Soltero'),
    ('Casado'),
    ('Divorciado'),
    ('Viudo');
...
-- ===========================================
-- SP PARA PERSPECTIVA POSTUALNTE
-- ===========================================
...
CREATE PROCEDURE sp_loginUsuario(
    ...
    IN p_contrasenaHash VARCHAR(128)
)
BEGIN
    SELECT 
        u.IdUsuario,
        ...
        u.ApellidoPaterno,
        u.ApellidoMaterno,
        u.IdPerfil,   -- aquí ya lo agregamos
        p.Nombre AS Perfil
    FROM Usuario u
    INNER JOIN Perfil p ON u.IdPerfil = p.IdPerfil
    WHERE u.Usuario = p_usuario
      AND u.ContrasenaHash = p_contrasenaHash
      AND u.IdEliminado = 0;
END //
DELIMITER ;


-- SP PARA CREAR USUARIO POSTULANTE Y VALIDAR QUE EL USUARIO EXISTA

DELIMITER //
CREATE PROCEDURE sp_registrarUsuarioPostulante(
    IN p_usuario VARCHAR(100),
...
BEGIN
    ...
    -- Verificar si el usuario ya existe y no está eliminado
    SELECT COUNT(*) INTO usuario_existe 
    FROM Usuario 
    WHERE Usuario = p_usuario AND IdEliminado = 0;

    IF usuario_existe > 0 THEN
        SET p_resultado = 2; -- usuario ya existe
    ELSE
        -- Insertar usuario (hash de contraseña aquí)
        INSERT INTO Usuario (Usuario, ContrasenaHash, Nombre, ApellidoPaterno, ApellidoMaterno, IdPerfil)
        VALUES (p_usuario, SHA2(p_contrasena, 512), p_nombre, p_apellidoPaterno, p_apellidoMaterno, 2);
        SET p_resultado = 1; -- usuario creado exitosamente
    END IF;
END //
DELIMITER ;
...
DELIMITER //
CREATE PROCEDURE sp_getProvincias(IN p_idDepartamento VARCHAR(2))
BEGIN
...
DELIMITER //
CREATE PROCEDURE sp_getDistritos(IN p_idProvincia VARCHAR(4))
BEGIN
...
DELIMITER //
CREATE PROCEDURE sp_getTipoDocumentos()
BEGIN
...
DELIMITER //
CREATE PROCEDURE sp_getGeneros()
BEGIN
...
DELIMITER //
CREATE PROCEDURE sp_getEstadosCiviles()
BEGIN
...
-- SP PARA VERIFICAR SI ES EL PRIMER INGRESO DEL PERFIL POSTULANTE 

DELIMITER //
CREATE PROCEDURE sp_esPrimerIngreso(
    IN p_IdUsuario INT,
    OUT p_esPrimerIngreso TINYINT
)
BEGIN
    DECLARE total INT;
    ...
    IF total = 0 THEN
        SET p_esPrimerIngreso = 1; -- primera vez
    ELSE
        SET p_esPrimerIngreso = 0; -- ya tiene registro
    END IF;
END //
DELIMITER ;


-- SP PARA CREAR POSTULANTE

DELIMITER //
CREATE PROCEDURE sp_guardarPerfilPostulante(
    IN p_IdUsuario INT,
    ...
    IN p_IdDepartamento VARCHAR(2),
    IN p_IdProvincia VARCHAR(4),
    IN p_IdDistrito VARCHAR(6),
    IN p_Telefono VARCHAR(20),
    IN p_Celular VARCHAR(20),
    IN p_Foto LONGBLOB,
    IN p_IdUsuarioCreacion INT,
    OUT p_resultado INT
)
BEGIN
    DECLARE existing INT;

    -- Validar si ya existe un postulante para este usuario
    SELECT COUNT(*) INTO existing
    FROM Postulante
    WHERE IdUsuario = p_IdUsuario AND IdEliminado = 0;

    IF existing > 0 THEN
        SET p_resultado = 0; -- Ya existe
    ELSE
        ...
        SET p_resultado = 1; -- Guardado exitoso
    END IF;
END //
DELIMITER ;

-- SP OBTENER USUARIO Y POSTULANTE:


DELIMITER //
CREATE PROCEDURE sp_obtenerDatosPostulante(IN p_idUsuario INT)
BEGIN
    SELECT 
        u.IdUsuario,
        u.Nombre,
        ...
        p.Foto
    FROM Usuario u
    INNER JOIN Postulante p ON u.IdUsuario = p.IdUsuario
    WHERE u.IdUsuario = p_idUsuario AND p.IdEliminado = 0;
END //
DELIMITER ;

-- Actualizar perfil del postulante (Usuario + Postulante)

DELIMITER //
CREATE PROCEDURE sp_actualizarDatosPostulanteyUsuario(
    IN p_idUsuario INT,
    IN p_nombre VARCHAR(50),
    IN p_apellidoPaterno VARCHAR(50),
    IN p_apellidoMaterno VARCHAR(50),
    ...
    IN p_celular VARCHAR(20),
    IN p_foto LONGBLOB,
    IN p_idUsuarioModificacion INT,
    OUT p_resultado INT
)
BEGIN
    ...
    SET Nombre = p_nombre,
        ...
        ContrasenaHash = IF(p_contrasena = '', ContrasenaHash, SHA2(p_contrasena, 512)),
    ...
    SET IdTipoDocumento = p_idTipoDocumento,
        ...
        Celular = p_celular,
        Foto = CASE 
                  WHEN p_foto IS NULL OR LENGTH(p_foto) = 0 THEN Foto 
                  ELSE p_foto 
               END,
        IdUsuarioModificacion = p_idUsuarioModificacion,
        FechaModificacion = NOW()
...
-- SP para listar convocatorias vigentes


DELIMITER //
CREATE PROCEDURE sp_listarConvocatoriasVigentes(
    IN p_idPostulante INT,
    IN p_idArea INT
)
BEGIN
    SELECT 
        ...
        c.FechaFin,
        a.Nombre AS Area,
        IF(p.IdPostulacion IS NULL, 0, 1) AS YaPostulado,
        IFNULL(p.CVPath, '') AS CVPath
    FROM Convocatoria c
    INNER JOIN Area a ON c.IdArea = a.IdArea
    LEFT JOIN Postulacion p 
        ON p.IdConvocatoria = c.IdConvocatoria 
        AND p.IdPostulante = p_idPostulante
        AND p.IdEliminado = 0
    WHERE c.FechaInicio <= CURDATE()
      AND c.FechaFin >= CURDATE()
      AND c.IdEliminado = 0
      AND (p_idArea = 0 OR c.IdArea = p_idArea) --  filtro opcional
...
-- Guardar postulación (adjuntando CV)


DELIMITER //
CREATE PROCEDURE sp_guardarPostulacion(
    IN p_idPostulante INT,
    IN p_idConvocatoria INT,
    IN p_cvPath VARCHAR(255),
    IN p_idUsuarioCreacion INT,
    OUT p_resultado INT
)
BEGIN
    ...
    IF NOT EXISTS(
        ...
        WHERE IdPostulante = p_idPostulante 
          AND IdConvocatoria = p_idConvocatoria
          AND IdEliminado = 0
    ) THEN
...
DELIMITER //
CREATE PROCEDURE sp_listarPostulaciones(IN p_IdPostulante INT)
BEGIN
    SELECT 
        po.IdPostulacion,
        c.IdConvocatoria,
        ...
        po.FechaCreacion
    FROM Postulacion po
    INNER JOIN Convocatoria c ON po.IdConvocatoria = c.IdConvocatoria
    WHERE po.IdPostulante = p_IdPostulante
      AND po.IdEliminado = 0
    ORDER BY po.FechaCreacion DESC;
END //
DELIMITER ;


-- SP RESUMEN POSTULANTE:

DELIMITER //
CREATE PROCEDURE sp_resumenPostulante(IN p_idPostulante INT)
BEGIN
    DECLARE v_convocatorias INT DEFAULT 0;
    DECLARE v_postulaciones INT DEFAULT 0;
    DECLARE v_ultimaPostulacion VARCHAR(50);

    -- Convocatorias vigentes
    SELECT COUNT(*) INTO v_convocatorias
    FROM (
    ...
    -- Total postulaciones del postulante
    SELECT COUNT(*) INTO v_postulaciones
    FROM Postulacion
    WHERE IdPostulante = p_idPostulante
      AND IdEliminado = 0;

    -- Última postulación (usamos FechaCreacion en tu tabla)
    SELECT DATE_FORMAT(MAX(FechaCreacion), '%d/%m/%Y %H:%i') INTO v_ultimaPostulacion
    FROM Postulacion
    WHERE IdPostulante = p_idPostulante
      AND IdEliminado = 0;

    -- Retorno
    SELECT v_convocatorias AS convocatoriasCount,
           v_postulaciones AS postulacionesCount,
           IFNULL(v_ultimaPostulacion, 'Aún no registras postulaciones') AS ultimaPostulacion;
END //
DELIMITER ;



-- SP PARA TRAER LA CONVOCATORIA POSTULADA:

	DELIMITER //
	CREATE PROCEDURE sp_ConvocatoriaDetalle(IN p_idConvocatoria INT)
	BEGIN
		SELECT 
			c.IdConvocatoria,
			c.Titulo,
		...
		FROM Convocatoria c
		INNER JOIN Area a ON c.IdArea = a.IdArea
		WHERE c.IdConvocatoria = p_idConvocatoria
		  AND c.IdEliminado = 0;
	END //
	DELIMITER ;
    
-- ===========================================
-- SP PARA PERSPECTIVA ADMINSITRADOR
-- ===========================================

-- Obtener áreas (todas o activas)
DELIMITER //
CREATE PROCEDURE sp_admin_obtener_areas(IN incluir_inactivas BOOLEAN)
BEGIN
    IF incluir_inactivas THEN
        SELECT * FROM Area;
    ELSE
        SELECT * FROM Area WHERE IdEliminado = 0;
    END IF;
END //
DELIMITER ;
...
DELIMITER //
CREATE PROCEDURE sp_admin_obtener_area_por_id(IN p_IdArea INT)
BEGIN
    SELECT * FROM Area WHERE IdArea = p_IdArea;
END //
DELIMITER ;

-- crear áreas 
DELIMITER //
CREATE PROCEDURE sp_admin_crear_area(
...
BEGIN
    INSERT INTO Area (Nombre, Descripcion, IdUsuarioCreacion, FechaCreacion)
    VALUES (p_Nombre, p_Descripcion, p_IdUsuarioCreacion, NOW());
END //
DELIMITER ;

-- actualizar areas
DELIMITER //
CREATE PROCEDURE sp_admin_actualizar_area(
    IN p_IdArea INT,
    IN p_Nombre VARCHAR(255),
    IN p_Descripcion TEXT,
    IN p_IdUsuarioModificacion INT
...
DELIMITER //
CREATE PROCEDURE sp_admin_buscar_area_por_nombre(IN p_nombre VARCHAR(255))
BEGIN
    SELECT *
    FROM Area
...
-- obtener historial de Areas eliminadas
DELIMITER //
CREATE PROCEDURE sp_admin_obtener_historial_eliminados()
BEGIN
    SELECT 
        a.IdArea,
        a.Nombre,
        ...
        a.FechaModificacion,
        a.FechaCreacion,
        um.Nombre AS ModificadorNombre,
        um.ApellidoPaterno AS ModificadorApellidoPaterno,
        um.ApellidoMaterno AS ModificadorApellidoMaterno,
        uc.Nombre AS CreadorNombre,
        uc.ApellidoPaterno AS CreadorApellidoPaterno,
        uc.ApellidoMaterno AS CreadorApellidoMaterno
    FROM Area a
    LEFT JOIN Usuario um ON a.IdUsuarioModificacion = um.IdUsuario
    LEFT JOIN Usuario uc ON a.IdUsuarioCreacion = uc.IdUsuario
    WHERE a.IdEliminado = 1
    ORDER BY a.FechaModificacion DESC;
END //
DELIMITER ;

-- contar areas inactivas
DELIMITER //
CREATE PROCEDURE sp_admin_contar_areas_inactivas()
BEGIN
    SELECT COUNT(*) AS cantidad FROM Area WHERE IdEliminado = 1;
END //
DELIMITER ;

-- contar areas activas
DELIMITER //
CREATE PROCEDURE sp_admin_contar_areas_activas()
BEGIN
    SELECT COUNT(*) AS cantidad FROM Area WHERE IdEliminado = 0;
END //
DELIMITER ;
...
-- Obtener Convocatorias (solo algunos campos)
DELIMITER $$

CREATE PROCEDURE sp_admin_obtener_convocatorias()
BEGIN
    SELECT c.*, a.Nombre AS NombreArea
    FROM Convocatoria c
    LEFT JOIN Area a ON c.IdArea = a.IdArea
    WHERE c.IdEliminado = 0
    ORDER BY c.FechaInicio DESC;
END$$

DELIMITER ;

-- obtener convocatorias por id
DELIMITER //
CREATE PROCEDURE sp_admin_obtener_convocatoria_por_id(IN p_Id INT)
BEGIN
    SELECT * FROM Convocatoria
    WHERE IdConvocatoria = p_Id AND IdEliminado = 0;
END //
DELIMITER ;

-- crear convocatoria
DELIMITER //
CREATE PROCEDURE sp_admin_crear_convocatoria (
...
BEGIN
    INSERT INTO Convocatoria (Titulo, Descripcion, FechaInicio, FechaFin, IdArea, IdUsuarioCreacion)
    VALUES (p_Titulo, p_Descripcion, p_FechaInicio, p_FechaFin, p_IdArea, p_IdUsuarioCreacion);
END //
DELIMITER ;

-- actualizar convocatoria
DELIMITER //
CREATE PROCEDURE sp_admin_actualizar_convocatoria (
    IN p_IdConvocatoria INT,
    IN p_Titulo VARCHAR(200),
    IN p_Descripcion TEXT,
    IN p_FechaInicio DATE,
...
BEGIN
    UPDATE Convocatoria SET 
        ...
        IdUsuarioModificacion = p_IdUsuarioModificacion,
        FechaModificacion = NOW()
    WHERE IdConvocatoria = p_IdConvocatoria AND IdEliminado = 0;
END //
DELIMITER ;

-- buscar convocatoria por areas
DELIMITER $$

CREATE PROCEDURE sp_admin_buscar_convocatorias_por_area(IN idArea INT)
BEGIN
    SELECT
        C.IdConvocatoria AS ID,
        C.Titulo AS Titulo,
    ...
    JOIN Area AS A ON C.IdArea = A.IdArea
...
CREATE PROCEDURE sp_admin_contar_convocatorias_no_eliminadas()
BEGIN
...
-- listar usuarios - se debe excluir al usuario
DELIMITER //

CREATE PROCEDURE sp_admin_obtener_usuarios_excluyendo_actual(
    IN p_incluirInactivos BOOLEAN,
    IN p_idUsuarioActual INT
)
BEGIN
    SELECT * FROM Usuario
    WHERE (IdEliminado = 0 OR (IdEliminado = 1 AND p_incluirInactivos = TRUE))
      AND IdUsuario != p_idUsuarioActual;
END //
...
DELIMITER $$
CREATE PROCEDURE sp_admin_obtener_usuario_por_id(IN p_IdUsuario INT)
BEGIN
...
CREATE PROCEDURE sp_admin_crear_usuario(
    IN p_Usuario VARCHAR(100),
...
BEGIN
    INSERT INTO Usuario (Usuario, ContrasenaHash, Nombre, ApellidoPaterno, ApellidoMaterno, IdPerfil, IdUsuarioCreacion)
    VALUES (p_Usuario, p_ContrasenaHash, p_Nombre, p_ApellidoPaterno, p_ApellidoMaterno, p_IdPerfil, p_IdUsuarioCreacion);
END $$
...
CREATE PROCEDURE sp_admin_actualizar_usuario(
    IN p_IdUsuario INT,
    IN p_Usuario VARCHAR(100),
    IN p_Nombre VARCHAR(100),
    IN p_ApellidoPaterno VARCHAR(100),
...
CREATE PROCEDURE sp_admin_buscar_usuarios_por_nombre(IN p_nombre VARCHAR(100))
BEGIN
    SELECT * FROM Usuario
    WHERE (Nombre LIKE CONCAT('%', p_nombre, '%')
           OR ApellidoPaterno LIKE CONCAT('%', p_nombre, '%')
           OR ApellidoMaterno LIKE CONCAT('%', p_nombre, '%'))
      AND IdEliminado = 0;
END //
...
CREATE PROCEDURE sp_admin_contar_usuarios_activos()
BEGIN
    SELECT COUNT(*) AS totalActivos FROM Usuario WHERE IdEliminado = 0;
END //
...
CREATE PROCEDURE sp_admin_contar_usuarios_inactivos()
BEGIN
    SELECT COUNT(*) AS totalInactivos FROM Usuario WHERE IdEliminado = 1;
END //
...
CREATE PROCEDURE sp_admin_reactivar_usuario(
    IN p_IdUsuario INT,
    IN p_IdUsuarioModificacion INT
)
BEGIN
...
-- ** Agregados **
-- ********
-- Procedimiento listar usuarios eliminados    
DELIMITER $$

CREATE PROCEDURE sp_admin_obtener_usuarios_eliminados()
BEGIN
    SELECT 
        IdUsuario,
        Usuario,
        ...
        IdPerfil,
        IdEliminado,
        FechaModificacion AS FechaEliminado
    FROM Usuario
    WHERE IdEliminado != 0;
END$$
...
-- Listar seleccionados por convocatoria
DELIMITER $$

CREATE PROCEDURE sp_obtener_resultados_por_convocatoria(IN idConvocatoria INT)
BEGIN
    SELECT 
        c.Titulo AS Convocatoria,
        a.Nombre AS Area,
        CONCAT(us.ApellidoPaterno, ' ', us.ApellidoMaterno, ', ', us.Nombre) AS NombrePostulante,
        td.Abreviatura AS TipoDocumento,
        p.NumeroDocumento,
        p.Celular,
        ROUND(AVG(e.Calificacion), 2) AS NotaFinal
    FROM Postulacion pos
    INNER JOIN Convocatoria c ON pos.IdConvocatoria = c.IdConvocatoria
    INNER JOIN Area a ON c.IdArea = a.IdArea
    INNER JOIN Evaluacion e ON pos.IdPostulacion = e.IdPostulacion
    INNER JOIN Usuario u ON e.IdEvaluador = u.IdUsuario
    INNER JOIN Postulante p ON pos.IdPostulante = p.IdPostulante
    INNER JOIN Usuario us ON p.IdUsuario = us.IdUsuario
    LEFT JOIN TipoDocumento td ON p.IdTipoDocumento = td.IdTipoDocumento
    WHERE 
        pos.Estado = 'seleccionado'
        AND pos.IdEliminado = 0
        AND c.IdEliminado = 0
        AND a.IdEliminado = 0
        AND e.IdEliminado = 0
        AND p.IdEliminado = 0
    ...
    GROUP BY 
        c.Titulo, a.Nombre,
        us.ApellidoPaterno, us.ApellidoMaterno, us.Nombre,
        td.Abreviatura, p.NumeroDocumento, p.Celular
    ORDER BY 
        NombrePostulante;
END$$
...
-- Listar seleccionados completos
DELIMITER $$

CREATE PROCEDURE sp_obtener_todos_resultados_generales()
BEGIN
    SELECT 
        c.Titulo AS Convocatoria,
        a.Nombre AS Area,
    ...
    LEFT JOIN Distrito di ON p.IdDistrito = di.IdDistrito
    WHERE pos.Estado = 'seleccionado'
        AND pos.IdEliminado = 0
        AND c.IdEliminado = 0
        AND a.IdEliminado = 0
        AND e.IdEliminado = 0
    ...
    GROUP BY 
        c.Titulo, a.Nombre,
        us.ApellidoPaterno, us.ApellidoMaterno, us.Nombre,
        td.Abreviatura, p.NumeroDocumento,
        p.Telefono, p.Celular, p.Direccion,
        d.NombreDepartamento, pr.NombreProvincia, di.NombreDistrito,
        u.Nombre
    ORDER BY c.Titulo, NombrePostulante
    LIMIT 1000;
END $$

DELIMITER ;

-- Eliminar convocatoria con quien lo hizo
DELIMITER $$

CREATE PROCEDURE sp_eliminar_convocatoria_logica(
    IN p_IdConvocatoria INT,
    IN p_IdUsuarioModificacion INT
)
BEGIN
...
-- ===========================================
-- SP PARA PERSPECTIVA EVALUADOR
-- ===========================================

-- Listar convocatorias
DELIMITER $$
CREATE PROCEDURE sp_listar_convocatorias()
BEGIN
    SELECT c.IdConvocatoria, c.Titulo, c.Descripcion, c.FechaInicio, c.FechaFin, 
           a.Nombre AS Area, c.IdArea
    FROM Convocatoria c
    INNER JOIN Area a ON c.IdArea = a.IdArea
    WHERE c.IdEliminado = 0
...
DELIMITER $$
CREATE PROCEDURE sp_obtener_convocatoria_por_id(IN p_IdConvocatoria INT)
BEGIN
    SELECT *
    FROM Convocatoria
    WHERE IdConvocatoria = p_IdConvocatoria
...
DELIMITER $$
CREATE PROCEDURE sp_eliminar_convocatoria(
    IN p_IdConvocatoria INT
)
BEGIN
    UPDATE Convocatoria
...
DELIMITER $$
CREATE PROCEDURE sp_listarAreas1()
BEGIN
    SELECT 
        IdArea, 
        Nombre AS NombreArea,
        Descripcion
    FROM Area
    WHERE IdEliminado = 0;
END$$
...
BEGIN
    INSERT INTO Area (Nombre, Descripcion, IdUsuarioCreacion)
    VALUES (p_Nombre, p_Descripcion, p_IdUsuarioCreacion);
END$$
DELIMITER ;
...
DELIMITER $$
CREATE PROCEDURE sp_obtener_area_por_id(IN p_IdArea INT)
BEGIN
    SELECT * FROM Area WHERE IdArea = p_IdArea AND IdEliminado = 0;
END$$
DELIMITER ;

DELIMITER $$
CREATE PROCEDURE sp_actualizar_area(
    IN p_IdArea INT,
    IN p_Nombre VARCHAR(100),
    IN p_Descripcion TEXT,
    IN p_IdUsuarioModificacion INT
...
DELIMITER $$
CREATE PROCEDURE sp_eliminar_area(IN p_IdArea INT)
BEGIN
    UPDATE Area
    SET IdEliminado = 1
    WHERE IdArea = p_IdArea;
...
DELIMITER $$
CREATE PROCEDURE sp_listar_postulaciones_por_convocatoria(
    IN p_IdConvocatoria INT
)
BEGIN
    SELECT 
        ps.IdPostulacion,
        ps.IdPostulante, -- <--- agregado para detalle
        ps.IdConvocatoria,
        u.Nombre,
        u.ApellidoPaterno,
        u.ApellidoMaterno,
        ...
        (
            SELECT COUNT(*) 
            FROM Evaluacion e 
            WHERE e.IdPostulacion = ps.IdPostulacion 
              AND e.IdEliminado = 0
        ) AS TotalEvaluaciones
    FROM Postulacion ps
    INNER JOIN Postulante p ON ps.IdPostulante = p.IdPostulante
    INNER JOIN Usuario u ON p.IdUsuario = u.IdUsuario
    WHERE ps.IdConvocatoria = p_IdConvocatoria
      AND ps.IdEliminado = 0;
END$$
DELIMITER ;


DELIMITER $$
CREATE PROCEDURE sp_listar_evaluaciones_por_postulacion(IN p_IdPostulacion INT)
BEGIN
    SELECT 
        e.IdEvaluacion,
        e.IdPostulacion,
        ...
        e.FechaModificacion
    FROM Evaluacion e
    INNER JOIN Etapa et ON et.IdEtapa = e.IdEtapa
    INNER JOIN Postulacion po ON po.IdPostulacion = e.IdPostulacion
    WHERE e.IdPostulacion = p_IdPostulacion
      AND e.IdEliminado = 0
...
DELIMITER $$
CREATE PROCEDURE sp_listar_convocatorias_activas()
BEGIN
    SELECT c.IdConvocatoria,
           c.Titulo,
...
CREATE PROCEDURE sp_insertar_evaluacion(
    ...
    IN p_IdEvaluador INT,
    IN p_IdUsuarioCreacion INT
)
BEGIN
    -- variables locales
    DECLARE v_estado VARCHAR(30);
    DECLARE v_min_pass DECIMAL(5,2) DEFAULT 11.00; -- umbral para considerar "aprobada" una etapa (ajustable)
    DECLARE v_total_etapas INT DEFAULT 3;
    DECLARE v_count_pass INT DEFAULT 0;
    DECLARE v_exists INT DEFAULT 0;

    -- Manejo simple de errores: si ocurre algo, hacemos rollback y devolvemos señal
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
        ROLLBACK;
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Error al insertar evaluación (procedimiento).';
    END;

    START TRANSACTION;

    -- Verificar que la postulación existe
    SELECT COUNT(*) INTO v_exists FROM Postulacion WHERE IdPostulacion = p_IdPostulacion;
    IF v_exists = 0 THEN
        ROLLBACK;
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Postulación no encontrada.';
    END IF;

    -- Insertar la evaluación (Fecha = día actual)
    INSERT INTO Evaluacion (
        IdPostulacion, IdEtapa, Calificacion, Comentario, IdEvaluador, Fecha, IdUsuarioCreacion
    ) VALUES (
        p_IdPostulacion, p_IdEtapa, p_Calificacion, p_Comentario, p_IdEvaluador, CURDATE(), p_IdUsuarioCreacion
    );

    -- Bloqueamos la fila de la postulación para evitar condiciones de carrera
    SELECT Estado INTO v_estado
      FROM Postulacion
     WHERE IdPostulacion = p_IdPostulacion
     FOR UPDATE;

    -- Determinar cuántas etapas hay definidas (no eliminadas)
    SELECT COUNT(*) INTO v_total_etapas FROM Etapa WHERE IdEliminado = 0;
    IF v_total_etapas = 0 THEN
        SET v_total_etapas = 3; -- fallback si no hay etapas definidas por alguna razón
    END IF;

    -- Si la calificación está por debajo del umbral → rechazo directo
    IF p_Calificacion < v_min_pass THEN
        UPDATE Postulacion
           SET Estado = 'descartado',
               IdUsuarioModificacion = p_IdUsuarioCreacion,
               FechaModificacion = NOW()
         WHERE IdPostulacion = p_IdPostulacion;
    ELSE
        -- Si está "postulado", al obtener una evaluación aprobatoria lo ponemos en proceso
        ...
        -- Contar cuántas etapas distintas han sido aprobadas (calificación >= umbral)
        SELECT COUNT(DISTINCT IdEtapa) INTO v_count_pass
          FROM Evaluacion
         WHERE IdPostulacion = p_IdPostulacion
           AND Calificacion >= v_min_pass
           AND IdEliminado = 0;

        -- Si aprobó todas las etapas requeridas → seleccionado
        IF v_count_pass >= v_total_etapas THEN
            UPDATE Postulacion
...
CREATE PROCEDURE sp_obtener_postulacion(IN p_IdPostulacion INT)
BEGIN
    SELECT 
        po.IdPostulacion,
        po.IdConvocatoria,     
        ...
        po.CVPath
    FROM Postulacion po
    INNER JOIN Postulante p ON po.IdPostulante = p.IdPostulante
    INNER JOIN Usuario u ON p.IdUsuario = u.IdUsuario
    WHERE po.IdPostulacion = p_IdPostulacion;
END$$
...
BEGIN
    ...
    SET Calificacion = p_Calificacion,
        ...
        IdEtapa      = p_IdEtapa,
        IdEvaluador  = p_IdEvaluador,
        Fecha        = CURDATE(),
        FechaModificacion = NOW()
    WHERE IdEvaluacion = p_IdEvaluacion;

    -- Regla de negocio: si la calificación >= 10, sigue en proceso
    IF p_Calificacion >= 10 THEN
        UPDATE Postulacion
        SET Estado = 'en_proceso',
            FechaModificacion = NOW()
        WHERE IdPostulacion = p_IdPostulacion;
...
CREATE PROCEDURE sp_obtener_evaluacion_por_id(IN p_IdEvaluacion INT)
BEGIN
    SELECT 
        e.IdEvaluacion,
        e.IdPostulacion,
    ...
    INNER JOIN Postulacion po ON po.IdPostulacion = e.IdPostulacion
    INNER JOIN Etapa et ON et.IdEtapa = e.IdEtapa
    WHERE e.IdEvaluacion = p_IdEvaluacion;
END$$
...
CREATE PROCEDURE sp_rechazar_postulacion(
    IN p_IdPostulacion INT,
    IN p_IdUsuarioModificacion INT,
...
BEGIN
    ...
    FROM Postulacion
    WHERE IdPostulacion = p_IdPostulacion AND IdEliminado = 0;

    IF v_Exists = 0 THEN
        SET p_Resultado = 'Error: Postulación no encontrada o eliminada.';
        ROLLBACK;
    ...
    SET p_Resultado = 'Postulación rechazada exitosamente.';
    COMMIT;
END //
...
CREATE PROCEDURE sp_actualizar_estado_postulacion(IN p_IdPostulacion INT)
BEGIN
    ...
    SELECT COUNT(*) INTO desaprobadas
    FROM Evaluacion
    WHERE IdPostulacion = p_IdPostulacion
      AND Calificacion < 10;

    -- Reglas de negocio
    IF desaprobadas > 0 THEN
        UPDATE Postulacion 
        SET Estado = 'descartado',
            FechaModificacion = NOW()
        WHERE IdPostulacion = p_IdPostulacion;
    ELSEIF total_etapas >= 3 AND aprobadas = total_etapas THEN
        UPDATE Postulacion 
        SET Estado = 'seleccionado',
            FechaModificacion = NOW()
        WHERE IdPostulacion = p_IdPostulacion;
...
DELIMITER $$
CREATE PROCEDURE sp_contar_evaluaciones(IN p_IdPostulacion INT)
BEGIN
...
DELIMITER $$
CREATE PROCEDURE sp_listar_convocatorias_por_area(IN p_IdArea INT)
BEGIN
    DECLARE v_Resultado VARCHAR(255);

    -- Depuración: Registrar el valor de p_IdArea
    SET v_Resultado = CONCAT('Ejecutando sp_listar_convocatorias_por_area con p_IdArea=', COALESCE(p_IdArea, 'NULL'));

    -- Seleccionar convocatorias
    SELECT c.IdConvocatoria,
           c.Titulo,
           c.FechaInicio,
           c.FechaFin,
    ...
    INNER JOIN Area a ON c.IdArea = a.IdArea
    WHERE c.IdEliminado = 0
      AND (p_IdArea = 0 OR p_IdArea IS NULL OR c.IdArea = p_IdArea);

    -- Devolver mensaje de depuración (opcional, para verificar en pruebas)
    SELECT v_Resultado AS debug_message;
END $$
DELIMITER ;

DELIMITER $$
CREATE PROCEDURE sp_listar_postulaciones_por_documento (
    IN p_IdConvocatoria INT,
    IN p_NumeroDocumento VARCHAR(20)
)
BEGIN
    SELECT 
        ...
        (SELECT COUNT(*) 
         FROM Evaluacion e 
         WHERE e.IdPostulacion = p.IdPostulacion 
           AND e.IdEliminado = 0) AS TotalEvaluaciones
    FROM Postulacion p
    INNER JOIN Postulante po ON p.IdPostulante = po.IdPostulante
    INNER JOIN Usuario u ON po.IdUsuario = u.IdUsuario
    WHERE p.IdConvocatoria = p_IdConvocatoria
      AND p.IdEliminado = 0
      AND (
            p_NumeroDocumento IS NULL 
            OR p_NumeroDocumento = '' 
            OR po.NumeroDocumento LIKE CONCAT('%', p_NumeroDocumento, '%')
          );
END $$
DELIMITER ;

DELIMITER $$
CREATE PROCEDURE sp_detalle_postulante(
    IN p_IdPostulante INT
)
BEGIN
    SELECT 
        ...
        u.ApellidoMaterno,
        p.NumeroDocumento,
        td.NombreTipoDocumento AS TipoDocumento,
    ...
    LEFT JOIN Provincia pr ON p.IdProvincia = pr.IdProvincia
    LEFT JOIN Distrito di ON p.IdDistrito = di.IdDistrito
    WHERE p.IdPostulante = p_IdPostulante
      AND p.IdEliminado = 0;
END$$
DELIMITER ;
